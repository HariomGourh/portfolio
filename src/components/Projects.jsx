...
const PROJECTS = [
  {
    ...
    imgBg: 'linear-gradient(135deg,#0a1628,#1a2040)',
  },

  {
    ...
    imgBg: 'linear-gradient(135deg,#1a0a20,#0a1a10)',
  },

  {
    ...
    imgBg: 'linear-gradient(135deg,#0d1b2a,#1a0a2e)',
  },
]
...
function ProjectCard({ project, delay }) {
  ...
  useEffect(() => {
    ...
    const onMove = (e) => {
      ...
      card.style.transform = `perspective(900px) translateY(-8px) rotateY(${x*9}deg) rotateX(${-y*9}deg)`
    }
  ...
  return (
    <div
      ...
      style={{ transitionDelay: delay + 's', transformStyle:'preserve-3d', transition:'transform 0.15s ease, border-color 0.4s, box-shadow 0.4s, transform 0.4s' }}
    >
      {/* ── Thumbnail ── */}
      <div
        className="relative overflow-hidden flex items-center justify-center"
        style={{ height:220, background: project.imgBg, borderRadius:'20px 20px 0 0' }}
      >
        <div className="text-7xl select-none transition-transform duration-500 hover:scale-110">
          {project.emoji}
        </div>
      ...
      {/* ── Info ── */}
      <div className="p-7">
        <div className="font-mono text-xs tracking-widest uppercase mb-1" style={{ color:'var(--cyan)' }}>
          {project.tag}
        </div>
        <h3 className="font-display font-bold text-xl mb-3 transition-colors duration-300 hover:text-[color:var(--cyan)]"
        ...
        <div className="flex gap-5">
          <a href={project.demo} className="flex items-center gap-2 text-sm font-medium no-underline transition-colors duration-300 hover:text-[color:var(--cyan)]"
             style={{ color:'var(--muted)' }} target="_blank" rel="noreferrer">
            <LinkIcon /> Live Demo
          </a>
          <a href={project.code} className="flex items-center gap-2 text-sm font-medium no-underline transition-colors duration-300 hover:text-[color:var(--cyan)]"
             style={{ color:'var(--muted)' }} target="_blank" rel="noreferrer">
            <GithubIcon /> GitHub
          </a>
        </div>
